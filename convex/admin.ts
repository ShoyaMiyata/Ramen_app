...
// 全ユーザーまたは特定ユーザーにチケットを配布
export const distributeTickets = mutation({
  args: {
    adminUserId: v.id("users"),
    amount: v.number(),
  ...
  handler: async (ctx, args) => {
    ...
    // 対象ユーザー取得
    const targetUsers = args.targetUserIds
      ? await Promise.all(args.targetUserIds.map((id) => ctx.db.get(id)))
      : await ctx.db.query("users").collect();

    const validUsers = targetUsers.filter(
      (u) => u && !u.deletedAt
    ) as Array<{ _id: Id<"users"> }>;
    ...
    for (const user of validUsers) {
      ...
      if (existingTickets) {
        await ctx.db.patch(existingTickets._id, {
          ticketCount: existingTickets.ticketCount + args.amount,
          updatedAt: now,
        });
      } else {
...
// 管理者がガチャをテスト実行（バッジは獲得しない）
export const testGacha = mutation({
  args: {
    adminUserId: v.id("users"),
  ...
  handler: async (ctx, args) => {
    ...
    const count = Math.min(args.count ?? 1, 10); // 最大10回
    const results = [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (random < 0.5) rarity = 5; // 0.5%
      else if (random < 5) rarity = 4; // 4.5%
      else if (random < 20) rarity = 3; // 15%
      else if (random < 55) rarity = 2; // 35%
      else rarity = 1; // 45%

      // バッジ選択
      const badgesOfRarity = getBadgesByRarity(rarity as 1 | 2 | 3 | 4 | 5);
      const randomIndex = Math.floor(Math.random() * badgesOfRarity.length);
      const badge = badgesOfRarity[randomIndex];
...
// デイリーガチャリセット（管理者専用）
export const resetDailyGacha = mutation({
  args: {
    adminUserId: v.id("users"),
    targetUserId: v.id("users"),
  },
  handler: async (ctx, args) => {
    ...
    if (dailyRecord) {
      await ctx.db.delete(dailyRecord._id);
    }
...
// 天井カウンターリセット（管理者専用）
export const resetPityCounter = mutation({
  args: {
    adminUserId: v.id("users"),
    targetUserId: v.id("users"),
  },
...
// ガチャ統計を取得（管理者用）
export const getGachaStats = query({
  args: {
    adminUserId: v.id("users"),
  },
  handler: async (ctx, args) => {
    ...
    // 全ユーザーのチケット総数
    const allTickets = await ctx.db.query("gachaTickets").collect();
    ...
    // 全ガチャ履歴
    const allHistory = await ctx.db.query("gachaHistory").collect();
    const totalGachaDraws = allHistory.length;

    // レアリティ別の排出数
    const byRarity = {
      1: allHistory.filter((h) => h.rarity === 1).length,
      2: allHistory.filter((h) => h.rarity === 2).length,
      3: allHistory.filter((h) => h.rarity === 3).length,
      4: allHistory.filter((h) => h.rarity === 4).length,
      5: allHistory.filter((h) => h.rarity === 5).length,
    };

    // デイリーガチャ実行数
    const dailyGachaDraws = allHistory.filter(
      (h) => h.gachaType === "daily"
    ).length;
